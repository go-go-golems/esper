...
func Run(ctx context.Context, cfg Config, stdout, stderr io.Writer) error {
	...
	out, errOut, closeFn, err := newOutputs(cfg, stdout, stderr)
	if err != nil {
		return err
	}
	defer closeFn()
	...
	return runLoop(ctx, cfg, p, out, errOut)
}
...
func newOutputs(cfg Config, stdout, stderr io.Writer) (out *outputs, errOut io.Writer, closeFn func(), err error) {
	closeFn = func() {}
...
func runLoop(ctx context.Context, cfg Config, port serial.Port, out *outputs, errOut io.Writer) error {
	_ = errOut
